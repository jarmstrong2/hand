...
function YHat:updateOutput(input)
    ...
    self.e_t = (nn.Sigmoid():cuda()):forward(-self.hat_e_t)
    self.pi_t = (nn.SoftMax():cuda()):forward(self.hat_pi_t)
    self.mu_1_t = self.hat_mu_1_t:clone()
    self.mu_2_t =  self.hat_mu_2_t:clone()
    self.sigma_1_t = (nn.Exp():cuda()):forward(self.hat_sigma_1_t)
    self.sigma_2_t = (nn.Exp():cuda()):forward(self.hat_sigma_2_t)
    self.rho_t = (nn.Tanh():cuda()):forward(self.hat_rho_t)
