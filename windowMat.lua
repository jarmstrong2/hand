...
function Window:updateOutput(input)
    ...
    alphas_t = torch.exp(hat_alphas_t):cuda()
    betas_t = torch.exp(hat_betas_t):cuda()
    kappas_t = self.kappas_t_1 + torch.exp(hat_kappas_t):cuda()
    
    sampleSize = (#alphas_t)[1]
    
    u_vector = torch.linspace(1, self.cu_size, self.cu_size):cuda()
    ...
    output = torch.bmm(phi_t:cuda(), cu_resized:cuda()):squeeze(2)
...
function Window:updateGradInput(input, gradOutput)
    ...
    calc = torch.cmul(gradOutput_expanded:cuda(), cu_resized:cuda())
    ...
    self.gradInput = torch.cmul(self.gradInput:cuda(), self.mask:cuda())
